...
app.addHook('onRequest', async (request) => { requestStartTimes.set(request, process.hrtime.bigint()) })
app.addHook('onResponse', async (request, reply) => {
  const startedAt = requestStartTimes.get(request)
  if (startedAt === undefined) return
  const durationMs = Number(process.hrtime.bigint() - startedAt) / 1_000_000
  request.log.info({ requestId: request.id, method: request.method, route: request.routeOptions.url, statusCode: reply.statusCode, durationMs: Number(durationMs.toFixed(3)) }, 'Request completed')
})
...
const updateWorkflowSchema = z.object({ expectedVersion: z.number().int().min(1), name: z.string().trim().min(1).max(160).optional(), description: z.string().trim().max(4000).nullable().optional(), status: z.enum(['draft', 'active', 'paused', 'archived']).optional() }).strict().refine((value) => value.name !== undefined || value.description !== undefined || value.status !== undefined, { message: 'At least one workflow field must be provided.' })
...
function databaseUrl(): string { const value = process.env.DATABASE_URL; if (!value) { const error = new Error('DATABASE_URL is not configured'); error.name = 'DEPENDENCY_UNAVAILABLE'; throw error }; return value }
...
app.addHook('onClose', async () => { await Promise.all([repository?.close?.(), auditService?.close(), workflowRepository?.close()]) })
...
app.get('/ready', async (request, reply) => { try { await authRepository().ping(); return reply.send(apiSuccess({ status: 'ready', dependencies: { database: 'ok' } }, request.id)) } catch (error) { request.log.warn({ err: error }, 'Readiness dependency check failed'); return reply.code(503).send(apiFailure('DEPENDENCY_UNAVAILABLE', 'A required service is unavailable.', request.id)) } })

app.post<{ Body: RegisterRequest }>('/api/v1/auth/register', { config: { rateLimit: { max: 10, timeWindow: '5 minutes' } } }, async (request, reply) => {
  assertTrustedOrigin(request); const parsed = registerSchema.safeParse(request.body)
  ...
  try { const result = await authRepository().register(parsed.data); const session: AuthSession = { user: result.user, expiresAt: result.expiresAt }; reply.setCookie('virexa_session', result.sessionToken, sessionCookieOptions()); await audits().record({ organizationId: result.user.organizationId, actorUserId: result.user.id, action: 'identity.registered', resourceType: 'user', resourceId: result.user.id, requestId: request.id, metadata: { role: result.user.role } }); return reply.code(201).send(apiSuccess(session, request.id)) } catch (error: any) { if (error?.code === '23505') return reply.code(409).send(apiFailure('EMAIL_ALREADY_REGISTERED', 'An account already exists for this organization and email.', request.id)); throw error }
})
app.post<{ Body: LoginRequest }>('/api/v1/auth/login', { config: { rateLimit: { max: 10, timeWindow: '5 minutes' } } }, async (request, reply) => {
  assertTrustedOrigin(request); const parsed = loginSchema.safeParse(request.body); if (!parsed.success) return reply.code(400).send(apiFailure('VALIDATION_ERROR', 'Email and password are required.', request.id)); const result = await authRepository().login(parsed.data.email, parsed.data.password); if (!result) return reply.code(401).send(apiFailure('INVALID_CREDENTIALS', 'Email or password is incorrect.', request.id)); const session: AuthSession = { user: result.user, expiresAt: result.expiresAt }; reply.setCookie('virexa_session', result.sessionToken, sessionCookieOptions()); await audits().record({ organizationId: result.user.organizationId, actorUserId: result.user.id, action: 'identity.login_succeeded', resourceType: 'session', requestId: request.id }); return reply.send(apiSuccess(session, request.id))
})
...
app.post<{ Body: CreateWorkflowRequest }>('/api/v1/workflows', async (request, reply) => { assertTrustedOrigin(request); const context = await requireAuthenticated(request, authRepository()); requirePermission(context, 'workflow:create'); const parsed = createWorkflowSchema.safeParse(request.body); if (!parsed.success) return reply.code(400).send(apiFailure('VALIDATION_ERROR', 'Workflow data is invalid.', request.id, parsed.error.flatten().fieldErrors)); const rawKey = request.headers['idempotency-key']; const parsedKey = idempotencyKeySchema.safeParse(typeof rawKey === 'string' ? rawKey : ''); if (!parsedKey.success) return reply.code(400).send(apiFailure('VALIDATION_ERROR', 'A valid Idempotency-Key header is required.', request.id, { idempotencyKey: ['Use a 16-255 character unique key.'] })); const result = await workflows().createIdempotent(context.user.organizationId, context.user.id, parsed.data, parsedKey.data); if (!result.replayed) await audits().record({ organizationId: context.user.organizationId, actorUserId: context.user.id, action: 'workflow.created', resourceType: 'workflow', resourceId: result.workflow.id, requestId: request.id }); return reply.code(result.replayed ? 200 : 201).send(apiSuccess(result.workflow, request.id)) })
app.get<{ Params: { workflowId: string } }>('/api/v1/workflows/:workflowId', async (request, reply) => { markSensitiveResponse(reply); const context = await requireAuthenticated(request, authRepository()); requirePermission(context, 'workflow:read'); const parsedId = workflowIdSchema.safeParse(request.params.workflowId); if (!parsedId.success) return reply.code(400).send(apiFailure('VALIDATION_ERROR', 'Workflow ID is invalid.', request.id)); const workflow = await workflows().getById(context.user.organizationId, parsedId.data); if (!workflow) return reply.code(404).send(apiFailure('NOT_FOUND', 'Workflow was not found.', request.id)); return reply.send(apiSuccess(workflow, request.id)) })
app.patch<{ Params: { workflowId: string }; Body: UpdateWorkflowRequest }>('/api/v1/workflows/:workflowId', async (request, reply) => {
  ...
  if (parsed.data.expectedVersion !== existing.version) return reply.code(409).send(apiFailure('WORKFLOW_CONFLICT', 'Workflow changed before this update could be applied.', request.id))
  if (parsed.data.status && !canTransitionWorkflowStatus(existing.status, parsed.data.status)) return reply.code(409).send(apiFailure('INVALID_STATE_TRANSITION', `Workflow cannot transition from ${existing.status} to ${parsed.data.status}.`, request.id))
  ...
  if (!workflow) return reply.code(409).send(apiFailure('WORKFLOW_CONFLICT', 'Workflow changed before this update could be applied.', request.id))
  await audits().record({ organizationId: context.user.organizationId, actorUserId: context.user.id, action: 'workflow.updated', resourceType: 'workflow', resourceId: workflow.id, requestId: request.id, metadata: { status: workflow.status, version: workflow.version } })
